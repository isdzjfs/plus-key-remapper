...
3. 保留悬浮窗权限，以便在后台启动映射的应用并保持按键回调及时送达；截图、相机快门等操作仍需要无障碍权限。
...
部分 OxygenOS 版本即使已设置“完全允许后台行为”，仍会冻结前台监听服务，导致按键
积压数秒后集中触发。Shizuku 监听就绪时会保留一个静态的 1 像素悬浮窗口，利用系统
对悬浮窗口的冻结豁免；窗口不可触摸、不可聚焦，不做动画或轮询，暂停、断连和停止
监听时会移除。缺少悬浮窗权限时仍可监听，但状态会提示后台可能延迟。
熄屏时厂商仍可能冻结应用并挂起广播。当生效的单击或长按映射包含自定义 Intent 时，
底层服务会在实际按下 Plus 键时先发送 `KEYCODE_WAKEUP`，让系统亮屏并恢复应用调度，
再由原来的 Binder 回调执行映射。该操作不解锁手机，也不会熄灭已点亮的屏幕；无此类
映射、按键检测模式及暂停监听时不会启用这条亮屏路径，空闲时不执行命令或轮询。
延迟超过两秒才送达的旧按键会被丢弃，避免系统恢复后集中执行过期操作。
